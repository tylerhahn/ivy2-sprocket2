...
# HP Sprocket 2 specific constants
START_CODE = 17167
...
# HP Sprocket 2 specific settings
AUTO_OFF_NEVER = 0
AUTO_OFF_3_MIN = 3
AUTO_OFF_5_MIN = 5
AUTO_OFF_10_MIN = 10
...
class GetStatusTask(BaseTask):
    ...
    def process_response(self, response):
        ...
        is_cover_open = (queue_flags >> 0) & 1
        is_no_paper = (queue_flags >> 1) & 1
        is_wrong_smart_sheet = (queue_flags >> 2) & 1
...
class GetPrintReadyTask(BaseTask):
    ...
    def get_message(self):
        base_message = get_base_message(COMMAND_PRINT_READY, self.flag, True)

        struct.pack_into('>I', base_message, 8, self.length)
...
def get_base_message(command, flag_1=False, flag_2=False):
    ...
    struct.pack_into('>H', message, 0, START_CODE)
    struct.pack_into('>H', message, 2, command)
    struct.pack_into('>B', message, 4, 0)
    struct.pack_into('>B', message, 5, 0)
    struct.pack_into('>B', message, 6, 0)
    struct.pack_into('>B', message, 7, 0)
